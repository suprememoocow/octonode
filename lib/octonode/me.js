...
(function() {
  ...
  Me = (function() {
    ...
    Me.prototype.getOrgMembership = function(org, cb) {
      return this.client.get("/user/memberships/orgs/" + org, null, function(err, s, b, h) {
        console.log('getOrgMembership: ', s);
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("User getMembership error"));
        } else {
          return cb(null, b, h);
        }
      });
